// offline mode data storage is not working
// working for sending sms for the  100% and 90% usages
  ...
  if (WiFi.status() != WL_CONNECTED) {
    ...
    if (!isOnline) {
      ...
      if (wasOffline) {
        ...
        float cloudUsage = fetchFirebaseData();  // read cloud
        if (cloudUsage >= 0) {
          if (totalUsage > cloudUsage) {
            // Device has more recent data
            Serial.println("📤 Local usage is higher → pushing to Firebase.");
            updateFirebase(totalUsage);
          } else if (totalUsage < cloudUsage) {
            // Cloud is more recent
            Serial.println("📥 Cloud usage is higher → updating local usage.");
            totalUsage = cloudUsage;
          } else {
            Serial.println("✔ Usage already synced.");
          }
        } else {
          Serial.println("⚠ Failed to fetch cloud usage, skipping sync.");
        }
